...
namespace AOC23.Day2
{
    public static class Problem1
    {
        ...
        public static void Solve(string inputPath, int maxR, int maxG, int maxB)
        {
            ...
            int sum = 0;
            for (int i = 0; i < games.Length; i++)
                if (IsGamePossible(games[i], maxR, maxG, maxB))
                    sum += (i + 1);
            
            Console.WriteLine(sum);
        }

        private static bool IsGamePossible(Game game, int maxR, int maxG, int maxB)
        {
            return game.MaxR <= maxR && game.MaxG <= maxG && game.MaxB <= maxB;
        }
